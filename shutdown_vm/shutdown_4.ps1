...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$ManagedIdentityClientId = "ManagedIdentityClientID" # Client ID of the User-Assigned Managed Identity
)
...
try {
    ...
    if (-not $connection) {
        throw "Failed to connect using User-Assigned Managed Identity"
    }
    
    Write-Output "Successfully connected using User-Assigned Managed Identity"
    ...
    if ($vmStatus.DisplayStatus -eq "VM running") {
        ...
        if ($currentHour -eq $WorkdayEndHour -or $currentHour -gt $WorkdayEndHour) {
