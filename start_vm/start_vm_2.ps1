...
function Test-Weekend {
    ...
    if ($isWeekend) {
        Write-Debug "Current day is weekend. VM will not be started."
        return $true
    }
...
function Test-StartTime {
    ...
    if ($currentHour -eq $StartHour -or $currentHour -ge $StartHour) {
        Write-Debug "Current time is $StartHour:00 UTC+1. VM should be started."
        return $true
    }
    
    Write-Debug "Current time is not $StartHour:00 UTC+1. No action taken."
    return $false
}
